...
//    @PostMapping("/login")
    public String login(@Validated @ModelAttribute LoginForm form, BindingResult bindingResult
        ...
        //로그인 성공 처리
        //쿠키에 시간 정보를 넣어주지 않으면 세션쿠키가 된다.(브라우저 종료시 모두 종료)
...
//    @PostMapping("/login")
    public String loginV2(@Validated @ModelAttribute LoginForm form, BindingResult bindingResult
        ...
        //SessionManager를 통해 회원 데이터를 관리하고 보관
...
//       sessionManager.expire(request);
    ...
    public String logoutV3(HttpServletRequest request){
        //세션이 있으면 있는 세션을 반환하고, 없으면 null을 반환
        HttpSession session = request.getSession(false);
        if (session != null) {
            session.invalidate();//세션과 데이터 삭제(서버에서 삭제하는 거임)
        }
        return "redirect:/";
    }
